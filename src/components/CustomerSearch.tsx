...
export function CustomerSearch({
  ...
  useEffect(() => {
    ...
    // If query matches the currently selected item exactly, don't search again
    if (value && query === `${value.xcus} - ${value.xorg}`) {
       return;
    }
      ...
      <div className="relative">
        ...
        <input
          ...
          onChange={(e) => {
            ...
            // clear selected value if user modifies it manually
            if (value && e.target.value !== `${value.xcus} - ${value.xorg}`) {
               onChange(null);
            }
          }}
          onClick={() => {
              ...
              {results.map((customer, index) => (
                <li key={customer.xcus || index}>
                  <button
                    ...
                    onClick={() => {
                      onChange(customer);
                      setQuery(`${customer.xcus} - ${customer.xorg}`);
                      setIsOpen(false);
                      setResults([]);
                    }}
                  >
