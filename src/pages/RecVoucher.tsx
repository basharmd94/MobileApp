...
import { Loader2, Filter, X, Calendar, User, TrendingUp, Hash, Receipt } from 'lucide-react';
...
export default function RecVoucher() {
  ...
  const { errorToast, successToast, showError } = useToast();
  ...
  const fetchReceipts = useCallback(async (currentLimit: number, isLoadMore = false, overrideFilters?: any) => {
    ...
    try {
      ...
      const res = await getCustomerReceipts({ limit: currentLimit, zid: payloadZid, customer: payloadCustomer || "", receipt_date: payloadDate || "" });
      const fetchedReceipts = res.receipts || [];
      setReceipts(prev => {
        if (!isLoadMore) return fetchedReceipts;
        if (fetchedReceipts.length >= prev.length) return fetchedReceipts;
        const newReceipts = fetchedReceipts.filter(
          (fo: CustomerReceipt) => !prev.some((po) => po.xvoucher === fo.xvoucher)
        );
        return [...prev, ...newReceipts];
      });
      setHasMore(fetchedReceipts.length >= currentLimit);
    } catch (error: any) {
  ...
  return (
    <div className="h-[100dvh] bg-bg-base flex flex-col relative max-w-md mx-auto shadow-2xl overflow-hidden md:max-w-full">
      <Header title="Receipt Voucher" bgColor="bg-white">
        <div className="mt-4">
          <BusinessTabs activeTab={zid} onChange={setZid} className="mb-3" />
        </div>
        <div className="flex justify-between items-center">
          <p className="text-[11px] text-text-muted font-medium">{receipts.length} receipt{receipts.length !== 1 ? 's' : ''} found</p>
          <button onClick={() => setFiltersExpanded(!filtersExpanded)}
            ...
            {(receiptDate || customer) && <span className="w-2 h-2 rounded-full bg-orange-500 ml-0.5"></span>}
          </button>
        </div>
        {filtersExpanded && (
            ...
            <div className="grid grid-cols-1 gap-3">
              <div>
                ...
                <div className="relative flex items-center">
                  ...
                  {receiptDate && <button type="button" onClick={() => setReceiptDate('')} className="absolute right-2 text-text-muted hover:text-text-main"><X className="w-4 h-4" /></button>}
                </div>
              </div>
            </div>
            <div>
              <label className="block text-[10px] font-semibold text-text-muted mb-1 ml-1">Customer</label>
              <CustomerSearch zid={zid} employeeId={employeeId} value={customer} onChange={setCustomer} placeholder="Search by ID, Name or Area" />
            </div>
            <div className="flex gap-2 pt-1 border-t border-orange-200/50 mt-3 !pt-3">
              <button onClick={() => { setReceiptDate(''); setCustomer(null); }}
                className="flex-1 py-2 text-[11px] font-bold text-text-secondary bg-white border border-ui-border rounded-lg hover:bg-gray-50 active:scale-[0.98] transition-all">Clear</button>
              <button onClick={handleSearch}
                className="flex-[2] py-2 text-[11px] font-bold text-white bg-orange-500 rounded-lg shadow-md shadow-orange-500/20 hover:bg-orange-600 active:scale-[0.98] transition-all disabled:opacity-50" disabled={loading}>Apply Filters</button>
            </div>
          </div>
        )}
      </Header>
      ...
      <main className="flex-1 p-4 overflow-y-auto w-full md:max-w-3xl md:mx-auto pb-24">
        {loading && receipts.length === 0 ? (
          ...
          <div className="space-y-4">
            {receipts.map((rct, i) => (
              <div key={`${rct.xvoucher}-${i}`} className="bg-[#fff7ed] border border-orange-100 p-3.5 rounded-[16px] shadow-[0_2px_10px_rgb(0,0,0,0.03)]">
                <div className="flex justify-between items-start mb-3">
                  <div className="flex-1 pr-2">
                    <h3 className="text-[13px] font-bold text-text-main flex items-start gap-1.5 leading-tight">
                      <User className="w-3.5 h-3.5 text-orange-400 shrink-0 mt-0.5" />{rct.xsub || "Unknown Customer"}--{rct.xshort || "Unknown Customer"}
                    </h3>
                  </div>
                  <div className="text-right shrink-0">
                    <span className="text-[12px] font-bold text-emerald-600 bg-emerald-50 px-2 py-1 rounded-lg border border-emerald-100 flex items-center gap-1">
                      <TrendingUp className="w-3 h-3" />৳{Math.abs(rct.xprime).toLocaleString()}
                    </span>
                  </div>
                </div>
                <div className="flex flex-col gap-1.5 mb-3 p-2.5 bg-white/60 rounded-[12px] border border-orange-50">
                  <div className="flex justify-between">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-[10px] font-medium text-text-secondary truncate">Voucher: <span className="font-bold text-text-main">{rct.xvoucher}</span></span>
                    </div>
                    {rct.xdate && (
                      <div className="flex items-center gap-1.5">
                ...
                <div className="flex items-center gap-1.5 mt-2 text-[10px] text-text-muted px-1">
                  <span className="w-4 h-4 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center shrink-0"><span className="text-[8px] font-bold">SP</span></span>
                  <span className="truncate">{rct.xname} ({rct.xsp})</span>
                </div>
              </div>
            ))}
            {hasMore && receipts.length > 0 && <LoadMoreButton loading={loadingMore} onClick={loadMore} />}
            {!hasMore && receipts.length > 0 && (
              <div className="text-center mt-6 mb-2 text-[11px] text-orange-900/40 font-medium">{receipts.length} receipts loaded.</div>
            )}
          </div>
        )}
      </main>
