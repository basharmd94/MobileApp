...
export default function DeliveryOrders() {
  ...
  const clearFilters = () => {
    ...
    setItem(null);
    setLimit(10);
    // Note: the fetch will be triggered by useEffect due to dependency changes when handleSearch is not called explicitly,
    // but React batching might race. Let's let the useEffect handle it, or we can just update states and they'll trigger `fetchOrders` via dependency.
  };
    ...
    <div className="h-[100dvh] bg-bg-base flex flex-col relative max-w-md mx-auto shadow-2xl overflow-hidden md:max-w-full">
      ...
      <header className="flex flex-col px-4 pt-8 pb-3 bg-white shadow-[0_2px_10px_rgb(0,0,0,0.02)] z-10 rounded-b-2xl shrink-0">
        <div className="flex items-center mb-4">
          ...
          <h1 className="flex-1 text-center text-[14px] font-bold text-text-main pr-8">Delivery Orders</h1>
        </div>
             ...
             <div className="grid grid-cols-2 gap-3">
               <div>
                  ...
                  <input 
                    type="text" 
                    value={xdornum}
                    onChange={(e) => setXdornum(e.target.value)}
                    placeholder="e.g. DO--490864"
                ...
                >
                  ...
                  {order.items && order.items.length > 0 && (
                    <div className="mt-2 bg-white/60 rounded-[12px] overflow-hidden border border-orange-50">
                       <button 
                          onClick={() => toggleExpand(order.xdornum)}
                          className="w-full flex items-center justify-between p-2.5 hover:bg-orange-50/50 transition-colors"
                       >
                  ...
                  <div className="flex gap-2 mt-3 pt-3 border-t border-orange-200/50">
                     <button
                       onClick={() => navigate('/pay-date', { state: { order } })}
                       className="flex-1 py-2 text-[10px] font-bold text-orange-600 bg-orange-50 border border-orange-200 rounded-lg hover:bg-orange-100 transition-colors"
                     >
                       Pay Date {order.xdatepay ? `(${order.xdatepay})` : ''}
                     </button>
                     <button
                       onClick={() => navigate('/sales-return', { state: { order } })}
                       className="flex-1 py-2 text-[10px] font-bold text-slate-600 bg-slate-50 border border-slate-200 rounded-lg hover:bg-slate-100 transition-colors"
                     >
                       Return
                     </button>
                  </div>
